...
DRESULT disk_read(BYTE drv, BYTE *buff, DWORD sector, UINT count)
{
    ...
    for (unsigned int i = 0; i < count; i++) {
      if (!(bp = bread(dev, sector + i))) { /* Read the correct block number */
          return RES_ERROR; 
      }
      memmove(buff + i * BSIZE, bp->data, BSIZE); /* Copy data from buffer to destination */
      brelse(bp);
    }
    return RES_OK; 
}
...
DRESULT disk_write(BYTE drv, const BYTE *buff, DWORD sector, UINT count)
{
    ...
    for (i = 0; i < count; i++) {
      if ((bp = bget(dev, sector + i))) { /* STUDENT_TODO: replace this */
        memmove(bp->data, buff + i * BSIZE, BSIZE); /* Copy data from source to buffer */
